...
export default function Page() {
  ...
  const [authType, setAuthType] = useState<'headers' | 'cookie' | 'none'>(
    'headers',
  )
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const url =
      endpoint === 'backend' ? `${API_URL}/auth/header` : `/api/auth/header`
    ...
    <div className="flex flex-col w-full">
      ...
      >
        <Select
          ...
          name="authType"
          onChange={(e) =>
            setAuthType(e.target.value as 'headers' | 'cookie' | 'none')
          }
        />
        <Select
          options={[
